...
// Types
export type ForgeMode = 'leet' | 'glitch' | 'mirror' | 'fancy' | 'zerowidth' | 'clap' | 'vaporwave' | 'redacted';
...
const transforms = {
  ...
  glitch: (text: string, opts: ForgeOptions, rng: () => number) => {
    const intensity = Math.max(1, Math.floor(opts.intensity / 5)); // 1-20
    return text.split('').map(char => {
      if (char === ' ') return char;
      let out = char;
  ...
  fancy: (text: string, opts: ForgeOptions, rng: () => number) => {
    // Select map based on intensity or seed
    ...
    return text.split('').map(char => {
      ...
      return idx !== -1 ? targetMap[idx * 2] + targetMap[idx * 2 + 1] : char; // Unicode surrogate pairs handling simplified
    }).join('');
  },

  zerowidth: (text: string, opts: ForgeOptions) => {
    if (opts.intensity > 50) {
      // Decode mode (if input seems to be hidden text)
...
export function transform(text: string, opts: ForgeOptions): string {
  ...
  switch (opts.mode) {
    ...
    case 'fancy': return transforms.fancy(text, opts, rng);
...
export const PRESETS = [
  { name: '1337 H4x0r', mode: 'leet', intensity: 80 },
  { name: 'Lite Leet', mode: 'leet', intensity: 30 },
  { name: 'Zalgo Chaos', mode: 'glitch', intensity: 80 },
  { name: 'Mild Glitch', mode: 'glitch', intensity: 20 },
  { name: 'Vaporwave', mode: 'vaporwave', intensity: 100 },
  { name: 'Clap Back', mode: 'clap', intensity: 100 },
  { name: 'Redacted', mode: 'redacted', intensity: 100 },
  { name: 'Hidden Message', mode: 'zerowidth', intensity: 0 },
];
